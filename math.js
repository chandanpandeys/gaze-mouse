...
export function ridge(samples,targetKey,featureKey,lambda){const p=samples[0][featureKey].length,A=Array.from({length:p},()=>Array(p).fill(0)),b=Array(p).fill(0);for(const s of samples){const f=s[featureKey],y=s[targetKey],weight=1+.35*Math.abs((targetKey==='x'?s.x:s.y)-.5)*2;for(let i=0;i<p;i++){b[i]+=weight*f[i]*y;for(let j=0;j<p;j++)A[i][j]+=weight*f[i]*f[j]}}for(let i=1;i<p;i++)A[i][i]+=lambda;return solve(A,b)}
